$(function(){
  ...
  beflow.NodeBoxView = beflow.NodeView.extend({
    ...
    color: function (value) {
      this.$(".module").css( "background-color", value );
    },
    initializeNative: function () {
      ...
      if (!this.Native){
        if (beflow.NativeNodes.hasOwnProperty(this.model.lazyLoadType)) {
          ...
          if (this.model.get("type") == "svr") {
            this.model.set({
              page: ""
            });
            this.color(this.model.parentGraph.get("info").servercolor);
          ...
          if (this.model.get("type") == "cln") {
            this.color( this.model.parentGraph.attributes.pages.getByName( this.model.get("page") ).get("color") );
          }
        } else {
    ...
    dragStop: function(event, ui){
      ...
      if (this._alsoDrag.length) {
        _.each(this._alsoDrag, function(el){
          var initial = el.data("ui-draggable-alsodrag-initial");
          var helper = el.data("ui-draggable-alsodrag-helper");
          var node = el.data("beflow-node-view");
          // Move other node
      ...
      this.model.parentGraph.trigger("updateGraph");
    },
