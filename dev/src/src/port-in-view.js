$(function(){
  ...
  beflow.PortInView = beflow.PortView.extend({
    tagName: "div",
    className: "port",
    ...
    dragstart: function (event, ui) {
      ...
      // Edge preview
      var edgePreview = new beflow.EdgeView();
      beflow.edgePreview = edgePreview;
      this.drag(event, ui);
      this.$('.plugend').show();
    ...
    drag: function (event, ui) {
      if (beflow.edgePreview) {
        var dragX = ui.offset.left + beflow.shownGraph.view.el.scrollLeft;
        var dragY = ui.offset.top + 8 + beflow.shownGraph.view.el.scrollTop;
        ...
        beflow.edgePreview.setPositions(positions);
        beflow.edgePreview.redraw();
      }
      // Don't drag module
      event.stopPropagation();
    },
    dragstop: function (event, ui) {
      ...
      // Edge preview
      beflow.edgePreview.remove();
      beflow.edgePreview = undefined;
    ...
    unplugdrag: function (event, ui) {
      if (beflow.edgePreview && this.unpluggingEdge) {
        var dragX = ui.offset.left + beflow.shownGraph.view.el.scrollLeft;
        var dragY = ui.offset.top + 6 + beflow.shownGraph.view.el.scrollTop;
      ...
      if (typeabbr === "int" || typeabbr === "num" || typeabbr === "flo" ) {
        ...
        input
          .spectrum({
            ...
            localStorageKey: "beflow.settings.colorPalette",
    ...
    manualinput: function (event) {
      try {
        ...
        if (saveToState) {
          ...
          // Set state to node server
          if (this.model.node.get("type") == "svr"); {
            var data = {
              id: this.model.node.get("id"),
              input: inputname,
              value: val
            };
            beflow.trigger("saveToState", data);
          }
        }
        this.model.node.receive(inputname, val);
      } catch (err) {
        this.$(".manualinput").children("input").val("");
