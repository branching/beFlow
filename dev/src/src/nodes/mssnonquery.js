  ...
  exports.Node = NodeModel.extend({
    ...
    inputparams: function(params, s){
      try {
        ...
        // update Usuarios set ciudad = '<<ciudad>>' where Nombre = '<<nombre>>'
        if (params && params != '!') {
          try {
...
if (global.side == "cln") {
  ...
  $(function(){
    ...
    beflow.NativeNodes["mssnonquery"] = beflow.NodeBoxNativeView.extend({
      template: _.template(template),
      info: {
        title: "mssnonquery",
        description: "a node to execute non query to SQL Server"
      },
      type: "svr",
      depen: {
        npm: "mssql"
